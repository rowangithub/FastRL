	...
	double verify (int n_samples, int steps) {
    	...
    	if (pModule != NULL) {
        	...
        	if (pFunc && PyCallable_Check(pFunc)) {
            	...
            	if (PyErr_Occurred())
                	PyErr_Print();
            	fprintf(stderr, "Cannot find model_check function \"%s\"\n", argv[2]);
            	return 0;
        	}
        	Py_XDECREF(pFunc);
        	Py_DECREF(pModule);
...
int main(int argc, char** argv)
{
	...
	unsafes error_zones;
	partial_states e1;
	e1[3] = std::make_pair(-1.0, -0.026179938765);
	error_zones.push_back(e1);
	partial_states e2;
	e2[3] = std::make_pair(0.026179938765, 1.0);
	error_zones.push_back(e2);
	partial_states e3;
	e3[1] = std::make_pair(-1.2, -1.0);
	error_zones.push_back(e3);
	partial_states e4;
	e4[1] = std::make_pair(1.0, 1.2);
	error_zones.push_back(e4);

	whole_states init_zones;
	init_zones.push_back (std::make_pair(0,0));
	init_zones.push_back (std::make_pair(0,0));
	init_zones.push_back (std::make_pair(0,0));
