...
StateNode::StateNode(ActionNode* _parentAct, State* _state, vector<SimAction*>& _actVect, double _reward, bool _isTerminal):
  ...
  for (int trajectory = rootOffset; trajectory < numRuns_; ++trajectory) {
    ...
    while (true) {
      ...
      if (current->isFull()) { //follow the UCT tree
        ...
        StateNode* nextNode = current->nodeVect_[actID]->addStateNode(sim_->getState(), sim_->getActions(), r, sim_->isTerminal());
...
double UCTPlanner::MC_Sampling(StateNode* node, int depth) {
  ...
  for (int i = 0; i < depth; i++) {
    ...
    int actID = sim_->MC_action(); //rand() % actions.size();
